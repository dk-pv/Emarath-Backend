...
/**
 * The Lost Leads report query (RPT-02.7): paging plus the period/team filters (AC3). `team`
 * accepts repeated params (`?team=Sales&team=Support`), coerced to a clean array (or dropped when
 * blank) by the shared `normalizeFilterValues`. `from`/`to` are ISO instants the client computes
 * in its own timezone (the creation window; `from` is the selected period, default "any time").
 * Validated before the service runs, so an invalid request is a clean 400 with nothing queried.
 */
export class LostLeadsQueryDto {
  ...
  /** Which lead date the `from`/`to` window applies to: creation (default) or the last status change — for a LOST lead, when it was lost. */
  @Transform(trim)
  ...
  /**
   * Lost-reason buckets to narrow to (the summary drill-down). Catalogue values, or the
   * literal `none` for leads lost with no recorded reason.
   */
  @Transform(({ value }: { value: unknown }): unknown =>
    normalizeFilterValues(value),
  )
  @IsArray({ message: 'reason must be one or more values' })
  @IsString({ each: true, message: 'each reason must be a string' })
  @MaxLength(MAX_VALUE_LENGTH, {
    each: true,
    message: `each reason must be at most ${MAX_VALUE_LENGTH} characters`,
  })
  @ArrayMaxSize(MAX_FILTER_VALUES, {
    message: `reason accepts at most ${MAX_FILTER_VALUES} values`,
  })
  @IsOptional()
  reason?: string[];
}
